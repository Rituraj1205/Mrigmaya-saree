...
function CollectionCard({
  ...
  const [form, setForm] = useState({
    ...
    displayOnHome: collection.displayOnHome !== false
  });
  ...
  return (
    <div className="admin-card bg-white border border-gray-100 rounded-2xl p-4 space-y-3 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Collection</p>
          <h3 className="text-lg font-semibold text-gray-900">{form.title || "Untitled"}</h3>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => onSave(form)}
          ...
          <button
            onClick={() => window.confirm("Delete this collection?") && onDelete()}
            className="px-3 py-2 rounded-xl border border-red-200 text-red-600 text-xs"
          >
            Delete
          </button>
        </div>
      ...
      <div className="grid md:grid-cols-[1.3fr,1fr] gap-4">
        <div className="space-y-2">
          <div className="rounded-xl overflow-hidden bg-gray-100 aspect-[16/9]">
            {form.heroImage ? (
              <img src={resolveAsset(form.heroImage)} alt={form.title} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-xs text-gray-400">No hero image</div>
            )}
          </div>
          <div className="flex gap-2 text-xs text-gray-600">
            <label className="px-3 py-2 rounded-xl border border-gray-200 cursor-pointer bg-white">
              Upload hero
              <input
                type="file"
                accept="image/*"
            ...
            <label className="px-3 py-2 rounded-xl border border-gray-200 cursor-pointer bg-white">
              ...
              <input
                ...
                onChange={(e) => {
                  const file = e.target.files?.[0];
                  if (file && onGalleryUpload) onGalleryUpload(file);
                  e.target.value = "";
                }}
              />
            </label>
          ...
          <div className="flex gap-2 items-center">
            ...
            <label className="flex items-center gap-2 text-xs text-gray-600">
              <input
                ...
                onChange={(e) => handleChange("displayOnHome", e.target.checked)}
              />
              Show on home
            </label>
            <input
              type="number"
              value={form.order}
              onChange={(e) => handleChange("order", Number(e.target.value) || 0)}
              className="border rounded-xl px-2 py-1 text-xs w-24"
              placeholder="Order"
            />
          </div>
        </div>
